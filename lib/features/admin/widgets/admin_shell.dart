...
/// Sidebar-on-desktop / drawer-on-mobile shell for the admin role.
///
/// Mirrors `DispatcherShell`, `DriverShell` and `UserShell`. Menu items not
/// built yet (Fleet Overview, Drivers, Reports) render a "coming soon"
/// panel rather than being hidden, so the full navigation model is visible
/// even before every screen exists.
...
class _AdminShellState extends State<AdminShell> {
  ...
  void _selectPage(int index) {
    ...
    if (_isMobile(context)) _scaffoldKey.currentState?.closeDrawer();
  }
    ...
    return AppBar(
      ...
      actions: [
        Container(
          ...
          child: const Row(
            children: [
              Icon(Icons.admin_panel_settings_outlined, size: 16, color: AppColors.textTertiary),
              SizedBox(width: 5),
              Text(
                'Admin',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: AppColors.textBody),
              ),
            ],
          ),
        ),
      ...
      child: const Row(
        children: [
          CircleAvatar(
            ...
            child: Icon(Icons.admin_panel_settings_outlined, size: 18, color: AppColors.primary),
          ),
          SizedBox(width: 9),
          Expanded(
            child: Text(
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.construction_outlined, size: 38, color: AppColors.textTertiary),
            const SizedBox(height: 14),
            Text(
              title,
